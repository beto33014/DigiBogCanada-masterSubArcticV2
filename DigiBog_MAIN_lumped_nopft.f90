...
! Single-valued variable (and model parameter) declarations.
  ...
  CHARACTER(LEN=40) :: data_file_name_010, & !IN Model run information file
                      ...
                      data_file_name_051, &  !IN annual mean growing season temperature values
                      data_file_name_052, &  !IN week begining growing season
                      data_file_name_053, &  !IN week ending growing season
                      ...
                      data_file_name_230    !OUT annual col production
...
!-------------------------------------------------------------------------------
  ...
  Annual_loop: DO
    ...
    READ (051, *) gsTemperature !beto, mean growing season temperature
    READ (052, *) growingSeasonBegin !beto, week growing season begins
    READ (053, *) growingSeasonEnd !beto, week growing season ends
    
    !WRITE (*,*) "Growing season temperature:", gsTemperature
    !WRITE (*,*) "Week gs begins:", growingSeasonBegin
    !WRITE (*,*) "Week gs end:", growingSeasonEnd 
    ...
    !  * Q10_anoxic**((temperature - base_temp) / 10.0)
      ...
      IF (net_rain_res /= 1) THEN 
        IF (new_net_rain == 1) THEN
          ...
          READ (020, *) rainfall
          !READ (030, *) temperature !beto, weekly temperature
      ...
      oxic_decay = oxic_decay_base &
          * Q10_oxic**((gsTemperature - base_temp) / 10.0) !beto, these are mean GS temperatures?
      anoxic_decay = anoxic_decay_base &
          * Q10_anoxic**((gsTemperature - base_temp) / 10.0) !beto, these mean GS temperatures?
      ...
      IF (net_rain_res /= 1) THEN 
        IF (net_rain_res == 3) THEN
          ...
          IF (sub_annual_counter >= growingSeasonBegin .AND. sub_annual_counter <= growingSeasonEnd) THEN !Beto, changed to reflect growing season
      ...
      IF (net_rain_res /= 1) THEN 
        IF (net_rain_res == 3) THEN
          !IF (sub_annual_counter >= 22 .AND. sub_annual_counter <= 35) THEN
           IF (sub_annual_counter >= growingSeasonBegin .AND. sub_annual_counter <= growingSeasonEnd) THEN
            ...
            IF (net_rain_res /= 1) THEN
              IF (net_rain_res == 3) THEN
                IF (sub_annual_counter >= growingSeasonBegin & 
                        .AND. sub_annual_counter <= growingSeasonEnd) THEN !Beto, changed the calc of the summer water table
      ...
      IF (sub_annual_counter >= growingSeasonBegin .AND. sub_annual_counter <= growingSeasonEnd) THEN !beto, only decomp during growing season
    ...
    New_layer_props: DO x = 1, x_extent
      DO y = 1, y_extent
        IF (activation_status(x, y) == "on") THEN
          ...
          !write(*,*) "column avg summer wt depth:", col_wt_depth_av_su(x, y)
          !write(*,*) "column avg year wt depth:", col_wt_depth_av(x, y)
          !write(*,*) "################################################"
          IF (col_wt_depth_av_su(x, y) > 66.8) THEN !Beto, using average summer (growing season) water tables
            layer_mass(x, y, (no_layers(x,y) - 1), 1) = 0.0000001
          ELSE
            layer_mass(x, y, (no_layers(x,y) - 1), 1) =& !Beto, using average summer (growing season) water tables
                     ((9.3 + (1.33 * col_wt_depth_av_su(x, y)) - (0.022&
                       * (col_wt_depth_av_su(x, y)**2.0)))**2.0) * 0.0001&
                       !Use mean annual value from annual weather
                       *((0.1575 * gsTemperature) + 0.009132) * ((growingSeasonEnd - growingSeasonBegin) / 52.0)!Beto, changed temperature to mean growing season and production reduced according to length of growing season
          END IF
          !write(*,*) "column litter production:", layer_mass(x, y, (no_layers(x,y) - 1), 1)
          WRITE(230, '(20F20.8)') layer_mass(x, y, (no_layers(x,y) - 1), 1)  !Beto, save annual production to file
